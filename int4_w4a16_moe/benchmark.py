...
Usage::
    ...
    python -m int4_w4a16_moe.benchmark --regime decode
"""
...
from .reference import make_w4a16_weights, moe_align_block_size_ref
...
def main():
    ...
    for gemm in (gate_up, down):
        for tag, M in regimes:
            ...
            ms, gbps = _bench_one(M, KIMI["E"], gemm["N"], gemm["K"], KIMI["TOP_K"], block_m=block_m)
            print(f"{tag:8} {gemm['tag']:8} {M:5d} {gemm['N']:6d} {gemm['K']:6d} {ms:9.4f} {gbps:8.1f}")
