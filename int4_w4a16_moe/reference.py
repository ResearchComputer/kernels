...
"""Slow but correct PyTorch reference for the INT4 W4A16 fused-MoE GEMM.

This is the numerical oracle the optimized Triton kernel is checked against
(issue #1, acceptance: match dequant-then-matmul within ``atol/rtol ~ 2e-2``).
It is intentionally written for clarity, not speed: explicit unpack -> dequant
-> grouped GEMM. The same ``moe_align_block_size`` helper is reused to build the
sorted/padded dispatch the Triton kernel consumes, so both paths see identical
routing.
"""
