...
def main():
    ...
    print(f"hipblaslt allowed: {getattr(torch.backends.cuda, 'preferred_blas_library', lambda: 'n/a')()}"
          if hasattr(torch.backends.cuda, "preferred_blas_library") else "hipblaslt: n/a")
    for dt in (torch.bfloat16, torch.float16):
        a = torch.randn(4096, 4096, device=dev, dtype=dt)
        b = torch.randn(4096, 11008, device=dev, dtype=dt)
        ms = _ms(lambda: a @ b)
        flop = 2 * 4096 * 4096 * 11008
        print(f"  matmul [4096,4096]x[4096,11008] {str(dt):16}: {ms:8.3f} ms  {flop/ms/1e9:7.1f} TFLOP/s")
    # full reference FFN at two sizes
    for M in (512, 4096):
        ...
        ms = _ms(lambda: (F.silu(x @ wg) * (x @ wu)) @ wd)
